...
hiddenWeight = np.random.uniform(low=-(1/np.sqrt(hidden)), high=(1/np.sqrt(hidden)), size=(785, hidden + 1))
outputWeight = np.random.uniform(low=-(1/np.sqrt(10)), high=(1/np.sqrt(10)), size=(hidden + 1, 10))
...
#runs weight deltas
hiddenDelta = np.zeros((hidden + 1, 785))
outputDelta = np.zeros((hidden + 1, 10))
...
for epoch in range(0, 10):
    ...
    #    print("outputVector shape: ", outputVector.shape)
        ...
        outputError = np.subtract(1.0, outputVector)  #not sure if this is the correct syntax
